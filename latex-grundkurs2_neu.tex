...
Die Energie beträgt \(\num{1.4e12}\) in der Einheit \(\si{\TeV}\), also \(\SI{1.4e12}{\tera\eV}\).

Für Geschwindigkeiten sind Werte von \(\SIrange{0}{3e8}{\meter\per\second}\) zulässig.
...
%\\
%\\
%f &= \frac{n c}{\ell} = \SI{31.5}{\MHz}
%\\
%L_{Collider} &= \frac{f N^2}{A}
%\ = \ \frac{N^2}{2 \pi r^2}
%\ = \ \SI{5.069e22}{\per\cm\squared\per\s}
%\end{align*}
