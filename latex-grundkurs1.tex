...
\begin{document}
	...
	\begin{frame}
		\begin{minipage}[0.5\textheight]{0.58\textwidth}
	...
	\section{Was ist \LaTeX{}?}
		...
		\begin{frame}{Was ist \LaTeX{}}
			...
			\begin{itemize}
				...
				\item Der Quelltext lässt sich Versionsverwalten (\textrightarrow\ git)
...
%		\end{frame}
		...
		\begin{frame}{Anmerkungen}
			...
			\vspace{0.2cm}
			\slideheading{Anmerkung:} Man kann \href{https://latexbase.com}{latexbase.com} oder \href{https://www.overleaf.com}{overleaf.com} zum live-mitcoden benutzen.
		\end{frame}
	...
	\section{Grundlagen von \LaTeX{} und \TeX}
		...
		\begin{frame}{Dokumentenklassen}
			\begin{itemize}
				...
				\item Ihr \emph{beschreibt} was ihr schreibt (z.\,B. was eine Überschrift ist)
			...
			\begin{description}
				...
				\item[beamer] Präsentationen wie die hier
			\end{description}
		\end{frame}
		...
		\begin{frame}{Syntax - Befehle und Umgebungen}
			...
			\begin{itemize}
				...
				\item \emph{Optionale} Parameter in eckigen ( [...] )
...
\end{document}
			\end{latexcode}
			\begin{textblock}{10}(7.75,10.75)
				\includegraphics[width=5.25cm]{images/erstes-dokument-scrartcl}
			\end{textblock}
		\end{frame}
...
\end{document}
			\end{latexcode}
			\begin{textblock}{10}(7.75,10.75)
				\includegraphics[width=5.25cm]{images/erstes-dokument-article}
			\end{textblock}
		\end{frame}
...
\section{Cliquen in Graphen}
  Cliquen haben reale Anwendungsfälle.
  ...
  \subsection*{Hinweise}
    Cliquen klingt wie klicken.
				...
				\vspace{0.2cm}\\
				\includegraphics[width=1.5\linewidth]{images/gliederung-beispiel}
			\end{columns}
		\end{frame}
...
aber ohne Einrückung.
		...
		\begin{frame}[containsverbatim]{Einfache Textformatierung}
			\slideheading{\LaTeX-Code:}
			\begin{latexcode}
Text kann \textbf{fett} oder \texttt{typewriter}
oder \textit{kursiv} oder einfach nur
\emph{hervorgehoben} sein.
...
\end{enumerate}
		...
		\begin{frame}[t]{Übung}
			Schachtelung eine Aufzählung, so wie hier:
			\vspace{0.2cm}\\
			\begin{itemize}
				\item Kartoffeln
				\begin{itemize}
		...
		\begin{frame}[t,containsverbatim]{Übung -- Auflösung}
			\begin{columns}[t]
				\column{.6\textwidth}
				\slideheading{\LaTeX-Code:}
				\begin{latexcode}
...
% Hat keinen wirklich Mehrwert, oder?
...
\begin{description}
	\item[Kile] Guter Editor für GNU/Linux (KDE).
	\item[AUCTeX] für Emacs-Benutzer
	\item[Texstudio] Editor für alle Betriebssysteme
\end{description}
			\end{latexcode}
...
\end{tabular}
		...
		\begin{frame}[t]{Übung}
			...
			\begin{tabular}{c|l}
				Symbol & Name \\
				\hline
				N      & \tiny\emph{Natürliche} Zahlen \\
				R      & \emph{Reelle} Zahlen
			\end{tabular}
		\end{frame}
	
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		
		\begin{frame}[containsverbatim]{Übung -- Auflösung}
			\slideheading{\LaTeX-Code:}
			\begin{latexcode}
\begin{tabular}{c|l}
	Symbol & Name \\
...
\end{tabularx}
			...
			\begin{tabularx}{\textwidth}{l|X}
				Spalte 1 & Spalte 2 \\ \hline
				Foo & Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed diam nonumy eirmod. \\
				Bar & Lorem ipsum [...]
			\end{tabularx}
		\end{frame}
...
\usepackage{graphicx}
% ...
\includegraphics[width=2cm]{images/gnu}
\includegraphics[width=1cm]{images/gnu}
			\end{latexcode}
	...
	\section{Mathematischer Textsatz}
		...
		\begin{frame}[containsverbatim]{Mathe-Umgebung}
			...
			\begin{itemize}
				\item Inline Mathe:
				\begin{itemize}
					\item Die \texttt{\$...\$} Umgebung: Stammt nicht aus \LaTeX, sondern aus \TeX{}, weit verbreitet
					\item Die \texttt{\textbackslash(...\textbackslash)} Umgebung: Stammt aus \LaTeX\ und funktioniert besser mit den \texttt{ams}-Paketen und z.B. in Überschriften
				\end{itemize}
				\item Eigenständiges Mathe:
				\begin{itemize}
					\item Die \texttt{\$\$...\$\$} Umgebung: Aus \TeX
					\item Die \texttt{\textbackslash[...\textbackslash]} Umgebung: Aus \LaTeX
				\end{itemize}
			\end{itemize}
		\end{frame}
		...
		\begin{frame}[containsverbatim]{Mathe-Umgebung}
			\slideheading{\LaTeX{}-Code:}
			\begin{latexcode}
Wir können im Text inline-Mathe, wie z.\,B.
\(x \cdot \sqrt{2} \) verwenden. Oder auch
Matheformeln als ganzen Block:
\[ \sum_{k=1}^n k = \frac{n(n+1)}{2} \]
			\end{latexcode}
			
			\slideheading{Ergebnis:}\\
			Wir können im Text inline-Mathe, wie z.\,B. \(x \cdot \sqrt{2} \)
		...
		\begin{frame}[containsverbatim]{Mathe-Umgebung}
			\slideheading{\LaTeX{}-Code:}
			\begin{latexcode}
Neben Summen ($\sum_1^n$) gibt es auch Integrale:
...
\end{align*}
 ...
 \end{cases}
		...
		\begin{frame}[t]{Finale Übung}
			Bilde dieses Dokument nach (inkl. Überschrift):
			\vspace{0.2cm}
			\begin{center}
				\includegraphics[width=\linewidth]{images/mathe-uebung-1}
			\end{center}
		\end{frame}
			
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		
		\begin{frame}[containsverbatim]{Finale Übung -- Auflösung}
			\slideheading{\LaTeX{}-Code:}\\
			{\footnotesize
			\begin{latexcode}
